...
CALL POMELO_BEFORE_DROP_PRIMARY_KEY(NULL, 'UserLevelProjection');
ALTER TABLE `UserLevelProjection` DROP PRIMARY KEY;

ALTER TABLE `UserLevelProjection` RENAME COLUMN `MaxLevel` TO `Level`;

UPDATE `UserLevelProjection` SET `Id` = ''
WHERE `Id` IS NULL;
SELECT ROW_COUNT();


ALTER TABLE `UserLevelProjection` MODIFY COLUMN `Id` varchar(32) CHARACTER SET utf8mb4 NOT NULL;

ALTER TABLE `UserLevelProjection` ADD CONSTRAINT `PK_UserLevelProjection` PRIMARY KEY (`Id`);
CALL POMELO_AFTER_ADD_PRIMARY_KEY(NULL, 'UserLevelProjection', 'Id');

CREATE INDEX `IX_UserLevelProjection_ApplicationUserId` ON `UserLevelProjection` (`ApplicationUserId`);

INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20250522013724_UserLevelProjection', '8.0.10');
...
CREATE TABLE `LicenseViolation` (
    ...
    `Limit` int NOT NULL,
    `Actual` int NOT NULL,
    `ErrorMessage` varchar(100) CHARACTER SET utf8mb4 NOT NULL,
...
INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
VALUES ('20250527065444_AddedLicenseViolation', '8.0.10');
