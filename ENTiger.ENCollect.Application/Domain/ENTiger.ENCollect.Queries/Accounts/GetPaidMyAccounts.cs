...
namespace ENTiger.ENCollect.AccountsModule
{
    ...
    ///
    /// </summary>
    public class GetPaidMyAccounts : FlexiQueryPagedListBridgeAsync<LoanAccount, GetPaidMyAccountsParams, GetPaidMyAccountsDto, FlexAppContextBridge>
    {
        protected readonly ILogger<GetPaidMyAccounts> _logger;
        protected GetPaidMyAccountsParams _params;
        ...
        public GetPaidMyAccounts(ILogger<GetPaidMyAccounts> logger, IRepoFactory repoFactory, ICustomUtility utility)
        {
            _logger = logger;
        ...
        /// <returns></returns>
        public virtual GetPaidMyAccounts AssignParameters(GetPaidMyAccountsParams @params)
        {
            _params = @params;
            return this;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public override async Task<FlexiPagedList<GetPaidMyAccountsDto>> Fetch()
        {
            _repoFactory.Init(_params);
            _flexAppContext = _params.GetAppContext();  //do not remove this line
            userId = _flexAppContext.UserId;

            user = await _repoFactory.GetRepo().FindAll<ApplicationUser>().Where(a => a.Id == userId).FirstOrDefaultAsync();

            userType = await fetchLoggedInUserTypeAsync(user);

            var projection = await Build<LoanAccount>().SelectTo<GetPaidMyAccountsDto>().OrderBy(a => a.CollectorAllocationExpiryDate).ToListAsync();
        ...
        protected override IQueryable<T> Build<T>()
        {
            IQueryable<T> query = _repoFactory.GetRepo()
                                    ...
                                    .Where(x => x.Paid == true && x.Attempted == true);
        ...
        public async Task<string> fetchLoggedInUserTypeAsync(ApplicationUser? loggedinUser)
        {
            ...
            {
                AgencyUser? user = await _repoFactory.GetRepo()
                                        .FindAll<AgencyUser>()
                                        .FirstOrDefaultAsync(x => x.Id == loggedinUser.Id);
    ...
    ///
    /// </summary>
    public class GetPaidMyAccountsParams : PagedQueryParamsDtoBridge
    {
