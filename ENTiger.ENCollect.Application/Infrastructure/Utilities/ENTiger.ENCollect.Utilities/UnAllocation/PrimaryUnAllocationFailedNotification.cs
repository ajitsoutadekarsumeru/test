...
{
    public class PrimaryUnAllocationFailedNotification : IMessageTemplate, IFlexUtilityService
    {
        ...
        {
            EmailSubject = "Bulk Primay UnAllocation Upload Rejected - " + TransactionId;

            if (unAllocationType?.ToLower() == "customerid level")
            {
                EmailMessage = "Dear User, <br><br>" +
                          "Your primary unallocation file having TransactionId - "
