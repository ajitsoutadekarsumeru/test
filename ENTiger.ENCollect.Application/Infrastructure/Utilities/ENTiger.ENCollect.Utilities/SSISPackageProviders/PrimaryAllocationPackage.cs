...
{
    public class PrimaryAllocationPackage : SSISPackageBase
    {
        private readonly ILogger<PrimaryAllocationPackage> _logger;
        ...
        public PrimaryAllocationPackage()
        {
            // Initialize with default or null values to avoid throwing exceptions
            _repoTenantFactory = FlexContainer.ServiceProvider.GetService<IFlexTenantRepository<FlexTenantBridge>>();
            _logger = FlexContainer.ServiceProvider.GetService<ILogger<PrimaryAllocationPackage>>();
        ...
        public PrimaryAllocationPackage(
            ILogger<PrimaryAllocationPackage> logger,
        ...
        protected override void SetParameters(BulkUploadFileDto dto)
        {
            ...
            string configPrefix = $"PrimaryAllocation_{AllocationType?.Trim().Replace(" ", "")}_{dto.AllocationMethod?.Trim().Replace(" ", "")}";
            ...
            _logger.LogInformation("End Setting parameters for Primary allocation");
        }
        ...
        {
            var sSISResultDto = new SSISResultDto();
            ...
            _logger.LogInformation(
                "Primary allocation Information: {PkgLocation}, allocationType: {AllocationType}, WorkRequestId: {CustomId}, " +
                "ExcelDestination: {ExcelDestinationService}, ExcelSource: {ExcelSource}, ParamDatabaseConnection: {ConnectionPath}, " +
                "SSISLogPath: {SSISLogPath}, ParamExcelTemplateSource: {ExcelTemplateSourceService}",
                pkgLocation, AllocationType, CustomId, ExcelDestinationService, excelSource, ConnectionPath, SSISLogPath, ExcelTemplateSourceService);
            ...
            _logger.LogInformation("dtExecPath: {DestinationtExecPath}, arguments: {Arguments}", DestinationtExecPath, arguments);
            ...
            try
            {
                ...
                _logger.LogInformation("DTExec process completed.");
                ...
                sSISResultDto.PackageExecResult = result.StandardError.Contains("DTSER_FAILURE") || !string.IsNullOrEmpty(result.StandardError)
                    ? "Failure"
                    : "Success";

                if (sSISResultDto.PackageExecResult == "Failure")
                {
