...
{
    public partial class ProcessPrimaryUnAllocationUploaded : IProcessPrimaryUnAllocationUploaded
    {
        private readonly ILogger<ProcessPrimaryUnAllocationUploaded> _logger;
        ...
        private PrimaryUnAllocationFile? _model;
        ...
        /// </summary>
        public ProcessPrimaryUnAllocationUploaded(
            ILogger<ProcessPrimaryUnAllocationUploaded> logger,
        ...
        /// <summary>
        /// Executes the primary unallocation process.
        /// </summary>
        /// <param name="eventData">Uploaded file event data.</param>
        /// <param name="serviceBusContext">Service bus context.</param>
        public async Task Execute(PrimaryUnAllocationUploadedEvent eventData, IFlexServiceBusContext serviceBusContext)
        {
            _logger.LogInformation("Process started for PrimaryUnAllocationUploaded: {EventId}", eventData.Id);
            ...
            _model = await _repoFactory.GetRepo().FindAll<PrimaryUnAllocationFile>().FlexNoTracking()
            ...
            var dataTable = _dataTableUtility.ExcelToDataTable(_rootFolderPath, _model.FileName, sheetName);
            ...
            _logger.LogInformation("Process completed for EventId: {EventId} with Condition: {Condition}", eventData.Id, _eventCondition);
            await this.Fire<ProcessPrimaryUnAllocationUploaded>(_eventCondition, serviceBusContext);
        }
        /// <summary>
        /// Sets necessary variables required for allocation file processing.
        /// </summary>
        /// <param name="eventData">Uploaded file event data.</param>
        private void SetVariables(PrimaryUnAllocationUploadedEvent eventData)
        {
            _logger.LogInformation($"PrimaryUnAllocationUploaded: Starting allocation file processing | DBType - {_databaseSettings.DBType}");
        ...
        /// </summary>
        /// <summary>
        /// Processes a valid primary unallocation file by inserting its data into the intermediate table
        /// and executing the required stored procedures.
        /// </summary>
        /// <param name="dbUtility">Database utility for executing operations.</param>
        /// <param name="dataTable">The data table containing valid records to be processed.</param>
        /// <param name="model">The file model containing metadata like WorkRequestId.</param>
        /// <param name="event">The event associated with the uploaded file.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        private async Task<bool> ProcessValidFileAsync(DataTable dataTable, PrimaryUnAllocationFile model, PrimaryUnAllocationUploadedEvent @event)
        {
            _logger.LogInformation($"[Start] PrimaryUnAllocationFile Processing valid file for WorkRequestId: {model.CustomId}");
            ...
            try
            {
                ...
                _logger.LogInformation($"[Success] PrimaryUnAllocationFile Completed processing for WorkRequestId: {model.CustomId}");
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"[Error] PrimaryUnAllocationFile Failed while processing WorkRequestId: {model.CustomId}. Error: {ex.Message}");
        ...
        /// </summary>
        private async Task InsertIntoIntermediateTableAsync(DataTable dataTable, PrimaryUnAllocationFile model)
        {
            var insertDto = new InsertIntoUnAllocationIntermediateTableRequestDto
            {
                ...
                TenantId = _tenantId,
                Table = dataTable,
                TableName = PrimaryUnAllocationStoredProcedureEnum.InsertIntermediateTable.Value
            };

            _logger.LogInformation($"Inserting data into {insertDto.TableName} for WorkRequestId: {insertDto.WorkRequestId}");
            await _dbUtility.InsertIntoUnAllocationIntermediateTable(insertDto);
        }

        /// <summary>
        /// Executes stored procedures for validation and processing.
        /// </summary>
        private async Task ExecuteStoredProcedures(PrimaryUnAllocationFile model)
        {
            ...
            {
                PrimaryUnAllocationStoredProcedureEnum.ValidateAccount.Value,
                PrimaryUnAllocationStoredProcedureEnum.UpdateFileStatus.Value,
                PrimaryUnAllocationStoredProcedureEnum.Update.Value
            })
            {
                request.SpName = sp;
                await _dbUtility.ExecuteSP(request);
                _logger.LogInformation($"Executed SP: {sp} for WorkRequestId: {model.CustomId}");
            }
            ...
            _model = await _repoFactory.GetRepo().FindAll<PrimaryUnAllocationFile>()
                                            .FlexNoTracking()
                                            .FirstOrDefaultAsync(x => x.Id == _fileId);
