...
namespace ENTiger.ENCollect.AgencyModule
{
    ...
    {
        public override string Id { get; set; } = "3a13183e019f64f9f24f97ab58347080";
        public override string FriendlyName { get; set; } = "RejectAgencyPlugin";

        protected string EventCondition = "";

        protected readonly ILogger<RejectAgencyPlugin> _logger;
        ...
        public RejectAgencyPlugin(ILogger<RejectAgencyPlugin> logger, IFlexHost flexHost, IRepoFactory repoFactory, IDiffGenerator diffGenerator)
        {
            _logger = logger;
            _flexHost = flexHost;
        ...
        public virtual async Task Execute(RejectAgencyPostBusDataPacket packet)
        {
            ...
            if (_model != null)
            {
                ...
                {
                    obj.RejectAgency(_flexAppContext?.UserId);
        ...
        private async Task GenerateAndSendAuditEventAsync(RejectAgencyPostBusDataPacket packet)
        {
            string jsonPatch;
            foreach (var obj in _model)
            {
