...
{
    public partial class RenewAgentPlugin : FlexiPluginBase, IFlexiPlugin<RenewAgentPostBusDataPacket>
    {
        public override string Id { get; set; } = "3a12e7a090850fc66cc36ddc5930cf3f";
        public override string FriendlyName { get; set; } = "RenewAgentPlugin";

        protected string EventCondition = "";

        protected readonly ILogger<RenewAgentPlugin> _logger;
        ...
        public RenewAgentPlugin(ILogger<RenewAgentPlugin> logger, IFlexHost flexHost, IRepoFactory repoFactory, IDiffGenerator diffGenerator)
        {
            _logger = logger;
            _flexHost = flexHost;
        ...
        public virtual async Task Execute(RenewAgentPostBusDataPacket packet)
        {
            ...
            _agencyUsers = await _repoFactory.GetRepo().FindAll<AgencyUser>().Where(m => packet.Cmd.Dto.agentIds.Contains(m.Id)).FlexNoTracking().ToListAsync();

            if (_model != null)
            {
                foreach (var obj in _model)
                {
                    obj.RenewAgent(_flexAppContext?.UserId, packet.Cmd.Dto.NewExpiryDate);
        ...
        private async Task GenerateAndSendAuditEventAsync(RenewAgentPostBusDataPacket packet)
        {
            string jsonPatch;
            foreach (var obj in _model)
            {
