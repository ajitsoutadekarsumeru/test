...
namespace ENTiger.ENCollect.AllocationModule.GetAllocationFileAllocationPlugins
{
    ...
    /// 
    /// </summary>
    public partial class CheckFileAccess : FlexiBusinessRuleBase, IFlexiBusinessRule<GetAllocationFileDataPacket>
    {
        ...
        public virtual async Task Validate(GetAllocationFileDataPacket packet)
        {
            ...
            if (!packet.HasError)
            {
                ...
                var primaryFiles = await _repoFactory.GetRepo().FindAll<PrimaryAllocationFile>()
                ...
                var secondaryFiles = await _repoFactory.GetRepo().FindAll<SecondaryAllocationFile>()
                                        .Where(x => x.CreatedBy == userId &&
                                            (x.FileName == packet.Dto.FileName || x.CustomId + FileExtension == packet.Dto.FileName))
                                        .ToListAsync();
